...
(** [Engine] is a module for functions*)
...
in a particular [state] *)
  let evaluate board (state : State.state) =
    ...
    if state.checkmateblack || state.checkmatewhite then 99999 * turn
    else if state.stalematewhite || state.stalemateblack then 0
    else score
  ...
  let rec minimax board (state : State.state) depth rec_move is_first =
    ...
    let game_over =
      state.stalematewhite || state.stalemateblack || state.checkmatewhite
      || state.checkmateblack
    in
    if depth = 0 || game_over then (evaluate board state, rec_move)
      ...
      if maximizing_player then (
        ...
        for i = 0 to length - 1 do
          ...
          let new_state = sc |> Board.updateState bc square2_occ in
