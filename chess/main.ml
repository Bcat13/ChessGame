...
let game_over_test (state : State.state) =
  state.checkmateblack || state.checkmatewhite || state.stalemateblack
  || state.stalematewhite
...
let engine_move_test (board : Board.b) (state : State.state) game_type i =
  try
    ...
    let temp_st = state |> Board.updateState board square2_occ in
...
let in_check_printer (state : State.state) =
  if state.inCheckwhite && state.turn = White then
    print_endline "White is in check!"
  else if state.inCheckblack && state.turn = Black then
...
let engine_move_random (board : Board.b) (state : State.state) game_type =
  ...
  let square2_occ = square2.occupant in
  Board.move board square1 square2 state true;
  state |> Board.updateState board square2_occ |> game_type false board
...
let engine_move_minimax depth (board : Board.b) (state : State.state) game_type
  ...
  let square2_occ = square2.occupant in
  Board.move board square1 square2 state true;
  state |> Board.updateState board square2_occ |> game_type depth false board
  ...
  | move -> (
      try
        ...
        | Move x ->
            ...
            state
            |> Board.updateState board square2_occ
