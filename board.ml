...
(**[update_piece_list] updates the piecelist of a board [b] by moving the piece 
  ...
  exists*)
let update_piece_list (board : b) (square1 : Square.square)
...
(** [reverse_update_tiles] updates the tiles of a board by moving the piece
  ...
  to [square2]*)
let reverse_update_tiles (board : b) (old : Square.o) (square1 : Square.square)
    (square2 : Square.square) (is_ep : bool) =
...
let update_tiles (board : b) (square1 : Square.square) (square2 : Square.square)
    (is_ep : bool) =
...
let get_square (board : b) str =
  ...
  if in_bounds file rank then board.tiles.(rank).(file) else raise InvalidSquare
...
let ic_by_pawn2 (tiles : Square.square array array) (row : int) (col : int)
  ...
  if in_bounds (row + y) (col + x) then
    ...
    | None -> false
    | Some x ->
        if x.color = turn then false else if x.name = Pawn then true else false
  else false
...
let ic_by_king (tiles : Square.square array array) (row : int) (col : int)
  ...
  if in_bounds (row + y) (col + x) then
    ...
    | None -> false
    | Some x ->
        if x.color = turn then false else if x.name = King then true else false
  else false

(** [in_check] is true if the King whose turn it is happens to be in check and 
  false otherwise.
...
let paths_for_pawn (board : b) (piece : Piece.piece) (c_incr : int)
  ...
  if in_bounds (row + r_incr) (col + c_incr) then
    if is_take then
      ...
      let square_to_check1 = board.tiles.(row + r_incr).(col + (c_incr / 2)) in
...
let bishop_next_moves (board : b) (piece : Piece.piece) =
  ...
  let move_br = path_for_piece board piece (-1 * color_int) (-1 * color_int) in
...
let dnr_in_check (board : b) (piece1 : Piece.piece) (turn : Piece.c)
  ...
  update_piece_list board square1 square2 false;
...
let dnr_in_check_ep (board : b) (piece1 : Piece.piece) (turn : Piece.c)
  ...
  let old_square1_location = piece1.location in
  update_piece_list board square1 square2 true;
...
let in_checkmate (board : b) (turn : Piece.c) =
...
let in_stalemate (board : b) (turn : Piece.c) =
...
let update_take (state : State.state) (square2 : Square.o) =
  ...
  | Some t ->
      ...
      else state.black_has_captured.(loc) <- state.black_has_captured.(loc) + 1
  | None -> ()

let update_state (board : b) (sq2_occupant : Square.o) (state : State.state) =
  let (new_turn : Piece.c) = if state.turn = White then Black else White in

  let new_in_check_white =
    if new_turn = White then in_check board new_turn else state.in_check_white
  in
  let new_in_check_black =
    if new_turn = Black then in_check board new_turn else state.in_check_black
  in
  let new_stalemate_white =
    if new_turn = White then in_stalemate board new_turn
    else state.stalemate_white
  in
  let new_stalemate_black =
    if new_turn = Black then in_stalemate board new_turn
    else state.stalemate_black
  in
  let new_checkmate_white =
    if new_turn = White then in_checkmate board new_turn
    else state.checkmate_white
  in
  let new_checkmate_black =
    if new_turn = Black then in_checkmate board new_turn
    else state.checkmate_black
  in

  let (st : State.state) =
    {
      turn = new_turn;
      in_check_white = new_in_check_white;
      in_check_black = new_in_check_black;
      stalemate_white = new_stalemate_white;
      stalemate_black = new_stalemate_black;
      checkmate_white = new_checkmate_white;
      checkmate_black = new_checkmate_black;
...
let rec promote_pawn (board : b) (state : State.state) (square2 : Square.square)
    (is_engine : bool) =
  let pawn =
...
let move (board : b) (square1 : Square.square) (square2 : Square.square)
  ...
  if is_valid_move square1 square2 state.turn then (
    ...
    update_piece_list board square1 square2 is_ep;
